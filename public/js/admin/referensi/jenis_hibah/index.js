$(document).ready(function () {
    ...
    let urlHapus = "/referensi/jenis_hibah/destroy";

    function openModal(data = null) {
        formInput.get(0).reset(); // Reset form input

        $("input[name='id']").val(data?.id || "");
        $("input[name='jenis_hibah']").val(data?.jenis_hibah || "");
        $("input[name='maksimum']").val(data?.maksimum || "");
        console.log(data?.dl_proposal);

        $("#dl_proposal").val(data?.dl_proposal || "");
        $("#dl_laporan").val(data?.dl_laporan || "");

        dlProposal.setDate(data?.dl_proposal, true);
        dlLaporan.setDate(data?.dl_laporan, true);
    ...
    var table = $("#dataTable").DataTable({
        ...
        columns: [
            ...
            },
            {
                data: "maksimum",
                render: (data, type, row) =>
                    `<span class="text-dark fw-semibold mb-0">Rp ${data}</span>`,
            },
            {
                data: "dl_proposal",
                render: (data, type, row) =>
    ...
    $("#dataTable").on("click", ".btn-edit", function () {
        let data = {
            id: $(this).data("id"),
            jenis_hibah: $(this).data("jenis_hibah"),
            maksimum: $(this).data("maksimum"),
            dl_proposal: $(this).data("dl_proposal"),
            dl_laporan: $(this).data("dl_laporan"),
        };
        openModal(data);
    });
