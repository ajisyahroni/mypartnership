                        ...
                        <div class="content">
                            <div class="container mt-4">
                                <form action="{{ route('implementasi.store') }}" enctype="multipart/form-data"
                                    ...
                                    @csrf
                                    <input type="hidden" name="id_ev" value="{{ @$dataImplementasi->id_ev }}">
                                    ...
                                    <div class="row mb-3 align-items-center" id="tgl_mulai_wrapper">
                                        ...
                                        <div class="col-sm-9">
                                            <input type="date" id="tanggal_mulai" name="tgl_mulai"
                                                value="{{ @$dataImplementasi->tgl_mulai }}"
                                    ...
                                    <div class="row mb-3 align-items-center" id="tgl_selesai_wrapper">
                                        ...
                                        <div class="col-sm-9">
                                            <input type="date" id="tanggal_selesai" name="tgl_selesai"
                                                value="{{ @$dataImplementasi->tgl_selesai }}"
                                        ...
                                        <div class="col-sm-9">
                                            @foreach ($bentuk_kerjasama as $bentuk)
                                                <div class="custom-check checkbox-lg mb-2">
                                                    <input class="custom-check-input" type="checkbox"
                                                        ...
                                                        {{ in_array($bentuk->nama, explode(',', $dataImplementasi->bentuk_kegiatan)) ? 'checked' : '' }}
                                            ...
                                            <div class="bentuk_kegiatan_wrapper d-none">
                                                <input type="text" class="form-control" id="bentuk_kegiatan"
                                                    value="{{ @$dataImplementasi->bentuk_kegiatan_lain ?? @$dataImplementasi->bentuk_kegiatan_lain }}"
                                    ...
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-sm-9">
                                            <textarea name="deskripsi_singkat" id="deskripsi_singkat" class="form-control" cols="30" rows="5"
                                                 placeholder="Masukkan deskripsi singkat kegiatan minimal 100 karakter dan maksimal 500 karakter...">{!! @$dataImplementasi->deskripsi_singkat !!}</textarea>
                                    ...
                                    <div class="row mb-4">
                                        <div class="col-sm-3 ">
                                            ...
                                            <label class="col-form-label fw-bold text-dark">Upload Bukti
                                                Pelaksanaan / Implementasi Kerja Sama
                                            </label><br>
                                            @if (count(@$logFileIMP) > 0)
                                                <button type="button"
                                                    class="btn btn-outline-primary btn-sm btn-detail-log"
                                                    data-log="{{ json_encode(@$logFileIMP) }}">Log
                                                    Draft</button>
                                            @endif
                                        </div>
                                        <div class="col-sm-9">
                                            ...
                                            @if (@$dataImplementasi->file_imp != null)
                                                <small style="font-size: 12px;"><a target="_blank" {{-- href="{{ asset('storage/' . @$dataImplementasi->file_imp) }}">Lihat --}}
                                                        href="{{ getDocumentUrl(@$dataImplementasi->file_imp, 'file_imp') }}">Lihat
                                    ...
                                    <div class="row mb-4">
                                        <div class="col-sm-3 ">
                                            <label class="col-form-label fw-bold text-dark">Upload Bukti
                                                ...
                                                <a href="{{ asset('template/template_ikuenam.docx') }}" class="btn btn-sm btn-primary">Download Format
                                                    Laporan</a>
                                            </label>
                                            <br>
                                            @if (count(@$logFileIkuenam) > 0)
                                                <button type="button"
                                                    class="btn btn-outline-primary btn-sm btn-detail-log"
                                                    data-log="{{ json_encode(@$logFileIkuenam) }}">Log
                                                    Draft</button>
                                            @endif
                                        </div>
                                        <div class="col-sm-9">
                                            ...
                                            @if (@$dataImplementasi->file_ikuenam != null)
                                                <small style="font-size: 12px;"><a target="_blank"
                                                        href="{{ asset('storage/' . @$dataImplementasi->file_ikuenam) }}">Lihat
                                                        Dokumen Sebelumnya</a></small><br>
                                            @endif
                                            <small class="text-muted" style="font-size: 12px;">Optional (tidak
                                                wajib
...
@push('scripts')
    ...
    <script src="{{ asset('js/implementasi/edit.js') }}"></script>
    <script>
        $(document).ready(function() {
            let pelaksana = '{{ @$dataImplementasi->pelaksana_prodi_unit }}';
            let deskripsi_singkat = '{{ @$dataImplementasi->deskripsi_singkat }}';
            let link_pub_internal = '{{ @$dataImplementasi->link_pub_internal }}';
            let link_pub_eksternal = '{{ @$dataImplementasi->link_pub_eksternal }}';

            let pic_kegiatan = '{{ @$dataImplementasi->pic_kegiatan }}';
            let nama_pic_kegiatan = '{{ @$dataImplementasi->nama_pic_kegiatan }}';
            let jabatan_pic_kegiatan = '{{ @$dataImplementasi->jabatan_pic_kegiatan }}';
            let telp_pic_kegiatan = '{{ @$dataImplementasi->telp_pic_kegiatan }}';

            let email_pic_internal = '{{ @$dataImplementasi->email_pic_internal }}';
            let nama_pic_internal = '{{ @$dataImplementasi->nama_pic_internal }}';
            let jabatan_pic_internal = '{{ @$dataImplementasi->jabatan_pic_internal }}';
            let telp_pic_internal = '{{ @$dataImplementasi->telp_pic_internal }}';

            let category = '{{ @$dataImplementasi->category }}';
            let id_mou = '{{ @$dataImplementasi->id_mou }}';
            let judul = '{{ @$dataImplementasi->judul }}';
            let lvl_unit = '{{ @$dataImplementasi->lvl_unit }}';
            let lvl_prodi = '{{ @$dataImplementasi->lvl_prodi }}';
            let lvl_fak = '{{ @$dataImplementasi->lvl_fak }}';
            let BentukKegiatanLain =
                `{{ in_array('Lain-lain', explode(',', $dataImplementasi->bentuk_kegiatan)) }}`;

            if (category) {
                $('input[name="category"][value="' + category + '"]').prop("checked", true).trigger(
                    'change')
            }

            if (pelaksana) {
                $('input[name="pelaksana_prodi_unit"][value="' + pelaksana + '"]').prop("checked", true).trigger(
                    'change')
            }

            if (lvl_fak) {
                $("#select_fakultas_wrapper").removeClass("d-none");
                $('#select_fakultas').val(lvl_fak).trigger('change')
            }

            if (lvl_prodi) {
                $("#select_prodi_wrapper").removeClass("d-none");
                $('#select_prodi').val(lvl_prodi).trigger('change')
            }

            if (lvl_unit) {
                $("#select_unit_wrapper").removeClass("d-none");
                $('#select_unit').val(lvl_unit).trigger('change')
            }

            if (id_mou) {
                $('#id_mou').val(id_mou).trigger(
                    'change')
            }
            ...
            if (link_pub_eksternal) {
                $('input[name="link_pub_eksternal"]').val(link_pub_eksternal)
            }
            if (link_pub_internal) {
                $('input[name="link_pub_internal"]').val(link_pub_internal)
            }
            if (email_pic_internal) {
                $('input[name="email_pic_internal"]').val(email_pic_internal)
            }
            if (nama_pic_internal) {
                $('input[name="nama_pic_internal"]').val(nama_pic_internal)
            }
            if (jabatan_pic_internal) {
                $('input[name="jabatan_pic_internal"]').val(jabatan_pic_internal)
            }
            if (telp_pic_internal) {
                $('input[name="telp_pic_internal"]').val(telp_pic_internal)
            }
            if (pic_kegiatan) {
                $('input[name="pic_kegiatan"]').val(pic_kegiatan)
            }
            if (nama_pic_kegiatan) {
                $('input[name="nama_pic_kegiatan"]').val(nama_pic_kegiatan)
            }
            if (jabatan_pic_kegiatan) {
                $('input[name="jabatan_pic_kegiatan"]').val(jabatan_pic_kegiatan)
            }
            if (telp_pic_kegiatan) {
                $('input[name="telp_pic_kegiatan"]').val(telp_pic_kegiatan)
            }

            if (BentukKegiatanLain) {
                $(".bentuk_kegiatan_wrapper").removeClass("d-none").fadeIn();
            }
        });
    </script>
@endpush
