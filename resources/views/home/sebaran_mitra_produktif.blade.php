    ...
    <div class="container">
        <div class="row align-items-start">
            ...
            <div class="col-12 col-md-6 mb-2">
                ...
                <h3 class="title-dashboard">Sebaran Mitra Aktif dan Produktif</h3>
            </div>

            <div class="col-12 col-md-4 mb-2">
                <label for="filterSebaranMitraProduktif" class="form-label fw-bold">Filter Lembaga:</label>
                <div class="input-group">
                    <select id="filterSebaranMitraProduktif" 
                            name="filterSebaranMitraProduktif" 
                            class="form-control select2">

                        <option value="Universitas">Universitas</option>

                        @foreach ($RefLembagaUMS as $lmbg)
                            <option value="{{ $lmbg->nama_lmbg }}"
                                    data-place_state="{{ $lmbg->id_lmbg }}"
                                    {{ @$q == $lmbg->nama_lmbg ? 'selected' : '' }}>
                                {{ $lmbg->nama_lmbg }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-12 col-md-2 mb-2">
                ...
                <button class="btn btn-success w-100" onclick="downloadSebaranMitraProduktif()">
            ...
            <div class="col-md-6 mb-3">
                <div class="ranking-card p-3 text-center">
                    ...
                    <div class="point-box platinum " onclick="detailKerma('Luar Negeri','Kerja Sama Luar Negeri')">
                        {{ round(@$KermaLNProduktif, 2) }}
                    </div>
                </div>
            </div>
        </div>
        <div id="mapid_produktif" style="width: 100%; height: 400px;background-color: white;"></div>
        <div class="no-data-sebaran-mitra-produktif2 d-none text-center text-muted"
...
<script>
    function downloadSebaranMitraProduktif(){
        const selectedOptionSebaran = $("#filterSebaranMitraProduktif").find("option:selected");
        const selectedFilter = selectedOptionSebaran.val();
        const placeState = selectedOptionSebaran.data("place_state");

        const urlDownload = 'home/download-excel-sebaran-mitra-produktif';
...
<script>
    ...
    const dataNegaraProduktif2 = @json($dataNegaraProduktif2);

    // Jika data kosong, tampilkan pesan "Tidak ada data"
    if (!dataNegaraProduktif2 || dataNegaraProduktif2.length === 0) {
        document.getElementById('mapid_produktif').style.display = "none";
        document.querySelector('.no-data-sebaran-mitra-produktif2').classList.remove('d-none');
    } else {
        document.getElementById('mapid_produktif').style.display = "block";
        document.querySelector('.no-data-sebaran-mitra-produktif2').classList.add('d-none');

        const mapProduktif = L.map('mapid_produktif', {
            minZoom: 1,
            zoomControl: true,
        ...
        const negaraDataMapProduktif = {};
        let minJumlah = Infinity;
        let maxJumlah = -Infinity;

        dataNegaraProduktif2.forEach(item => {
            ...
            if (!negaraDataMapProduktif[nama]) negaraDataMapProduktif[nama] = 0;
            negaraDataMapProduktif[nama] += item.jumlah;
        ...
        fetch("https://raw.githubusercontent.com/johan/world.geo.json/master/countries.geo.json")
            .then(resProduktif => resProduktif.json())
            .then(geojsonProduktif => {

                const hongKongFeatureProduktif = {
                    "type": "Feature",
                    "properties": {
                        "name": "Hong Kong"
                    },
                ...
                geojsonProduktif.features.push(hongKongFeatureProduktif);


                // Hitung kuartil untuk pembagian 5 gradasi
                const values = Object.values(negaraDataMapProduktif).sort((a, b) => a - b);
                ...
                const layerGeoProduktif = L.geoJson(geojsonProduktif, {
                    ...
                    style: function(feature) {
                        ...
                        const jumlah = negaraDataMapProduktif[nama] || 0;
                ...
                mapProduktif.fitBounds(layerGeoProduktif.getBounds());
