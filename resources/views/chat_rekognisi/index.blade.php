                    ...
                    <div class="card shadow-lg border-0 rounded-4">
                        <div class="card-header">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a href="{{ url()->previous() }}" class="btn btn-danger btn-sm shadow-sm">
                            ...
                            <div class="container mt-4">
                                <div class="row">
                                                        ...
                                                        @foreach ($users as $user)
                                                            <li class="list-group-item user-item d-flex align-items-center border-0 p-2"
                                                                ...
                                                                <div class="d-flex flex-column">
                                                                    <strong class="text-dark mb-1" style="font-size: 12px;">
                                                                        {{ $user->name }}
                                                                        @if ($user->getChatRekognisi && count($user->getChatRekognisi) > 0)
                                                                            <!-- Notifikasi Pesan Baru (Merah) -->
                                                                            <span class="badge bg-warning ms-2"
                                                                                style="font-size: 10px;">
                                                                                {{ count($user->getChatRekognisi) }} Pesan Baru
                                                                            </span>
                                                                        @endif
                                                                    </strong>
                                                                    ...
                                                                    <!-- Status Badge (Verifikator, Admin, Pengusul) -->
                                                                    <div class="mt-1">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <input type="hidden" id="id_rec" value="{{ @$id_rec }}">
...
@push('scripts')
        ...
        $(document).ready(function() {
            ...
            $('#userSearch').on('keyup', function() {
                var searchTerm = $(this).val()
                    .toLowerCase(); // Ambil nilai input pencarian dan ubah ke lowercase
                $('#user-list .user-item').each(function() {
                    // Ambil nama dan email dari masing-masing user-item
                    var userName = $(this).find('strong').text()
                        .toLowerCase(); // Nama user (dalam strong)
                    var userEmail = $(this).find('small').text()
                        .toLowerCase(); // Email user (dalam small)

                    // Menyembunyikan atau menampilkan item berdasarkan kecocokan dengan searchTerm
    ...
    <script src="{{ asset('js/chat_rekognisi/index.js') }}"></script>
@endpush
