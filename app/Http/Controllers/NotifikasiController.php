...
class NotifikasiController extends Controller
{
    ...
    public function getDataNotifikasi()
    {
        ...
        $dataNotifikasi = [
            'pengajuan_kerja_sama_mobile' => $this->badgeMobile($jumlahPengajuan),
            'pengajuan_kerja_sama'        => $this->badgeDesktop($jumlahPengajuan),
            'pengajuan_implementasi_mobile' => $this->badgeMobile($jumlahImplementasi),
            'pengajuan_implementasi'        => $this->badgeDesktop($jumlahImplementasi),
            'menu_kerja_sama'               => $this->badgeMenu($totalNotifikasi),
            'menu_kerja_sama_mobile'        => $this->badgeMobile($totalNotifikasi),
        ];
                ...
                $query->where(function ($q) use ($tanggalNull) {
                    $q->where(function ($sub) use ($tanggalNull) {
                        ...
                        $sub->where(function ($sub1) {
                            $sub1->where('status_verify_admin', '0');
                            $sub1->whereNull('status_verify_admin');
                        });
    ...
    protected function badgeMobile($count)
    {
        return $count > 0
    ...
    protected function badgeDesktop($count)
    {
        return $count > 0
            ? '<span class="badge rounded-pill bg-danger position-absolute start-100 translate-middle" style="top: 20px;">' . $count . '</span>'
            : '';
    }

    protected function badgeMenu($count)
    {
        return $count > 0
            ? '<span class="badge rounded-pill bg-danger position-absolute translate-middle" style="top: 20px; left: 90%!important;">' . $count . '</span>'
            : '';
    }



    public function getDataNotifikasiHibah()
    {
        $today = Carbon::today();
        ...
        // $queryAjuan->where('tbl_ajuan_hibah.id_hibah', 'HBH38299309c0e44d4f99454c8efe192641');
        ...
        foreach ($ajuanHibahList as $row) {
            ...
            $isVerif = [
                'kaprodi' => $row->date_verify_kaprodi !== null,
                'dekan' => $row->date_verify_dekan !== null,
                'admin' => $row->date_verify_admin !== null,
                'laporan' => $row->date_verify_laporan !== null,
                'tahap1' => $row->date_verify_tahap_satu !== null,
                'tahap2' => $row->date_verify_tahap_dua !== null,
                'selesai' => $row->date_selesai !== null,
            ];
            ...
            $notifCases = [
                ...
                'admin' => (
                    ...
                    ($status['revisi']['laporan'] == '1' || $status['revisi']['laporan'] == null)
                ) || (
                    $status['verifikasi']['laporan'] &&
    ...
    public function getDataNotifikasiRecognition()
    {
        $username = Auth::user()->username;
        $placeState = Auth::user()->place_state;
    ...
    public function getNotifikasiPartner()
    {
        $username = Auth::user()->username;
