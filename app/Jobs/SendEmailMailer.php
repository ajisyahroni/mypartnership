...
class SendEmailMailer implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // Ambil nilai reply_to, pastikan tidak kosong dan idealnya divalidasi
        $replyToEmail = $mailSetting['reply_to'] ?? '';
        $replyToName = $mailSetting['subjek_reply_to'] ?? '';
        ...
        $emailDev = MailSetting::where('is_active', '1')->first();
        $arrReceiver = array_map('trim', explode(',', $emailDev->email_receiver));
        if ($session == 'dev') {
            foreach ($arrReceiver as $key => $arrEmail) {
                $mail->addAddress($arrEmail);
            }
            // } else if ($session == 'prod') {
        } else if ($session == 'prod') {
            // $mail->addAddress('mutazfarisi1@gmail.com'); // dinamis per email
            // $mail->addAddress('askarimmm.hanan@gmail.com'); // dinamis per email
